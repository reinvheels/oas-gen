...
const Response = ({ status, response, spec }: ResponseProps) => {
    ...
    return html`
        <h5>${status}</h5>
        ${response.description ? html`<p>${response.description}</p>` : ''} ${schema ? Schema({ schema, spec }) : ''}
    `;
};
...
const Schema = ({ schema, spec }: SchemaProps) =>
    html` <div>
        ${schema.properties &&
...
const Property = ({ name, schema }: PropertyProps) => html` <span>${name}:</span> <b>${schema.type}</b> <br /> `;
